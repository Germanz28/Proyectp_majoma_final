...
include 'conn.php';
$productos = [];
$query = $conn->query("SELECT * FROM product WHERE categoria = 'accesorio'");
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    $productos[] = $row;
}
// 6 productos aleatorios para el mini carrusel
$carouselStmt = $conn->query("SELECT * FROM product WHERE categoria = 'accesorio' ORDER BY RAND() LIMIT 6");
  ...
  <header>
    <div class="container mt-4">
      <div class="navbar-custom" role="navigation" aria-label="Navegación principal">
        <a class="navbar-brand" href="accesorios.php">
          <img src="/proyecto_majoma/assets/img/logo.png" alt="logo">
          Tienda Majoma
        </a>
        <div class="nav-menu" role="menu">
          <a class="nav-link" href="tecnologia.php" style="color: #989a9cff;">Tecnología</a>
        </div>
